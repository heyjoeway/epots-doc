...
THREEx.DomEvents.prototype._getRelativeTouchXY = function(domEvent){
	var element = domEvent.target || domEvent.srcElement;
	if (element.nodeType === 3) {
		element = element.parentNode; // Safari fix -- see http://www.quirksmode.org/js/events_properties.html
	}
	...
	return {
		x : +((domEvent.touches[ 0 ].pageX - elPosition.x) / elDimension.width ) * 2 - 1,
		y : -((domEvent.touches[ 0 ].pageY - elPosition.y) / elDimension.height) * 2 + 1
	};
};
...
THREEx.DomEvents.prototype._onTouchMove	= function(domEvent)
{
	...
	var touchCoords = this._getRelativeTouchXY(domEvent);
	this._onMove('mousemove', touchCoords.x, touchCoords.y, domEvent);
	this._onMove('mouseover', touchCoords.x, touchCoords.y, domEvent);
	this._onMove('mouseout' , touchCoords.x, touchCoords.y, domEvent);
}
...
THREEx.DomEvents.prototype._onTouchEvent	= function(eventName, domEvent)
{
	...
	// var mouseX	= +(domEvent.touches[ 0 ].pageX / window.innerWidth ) * 2 - 1;
	// var mouseY	= -(domEvent.touches[ 0 ].pageY / window.innerHeight) * 2 + 1;
	this._onEvent(eventName, touchCoords.x, touchCoords.y, domEvent);	
}
